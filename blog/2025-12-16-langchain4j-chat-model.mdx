...
- **安全防护**：严禁用户输入直接拼接至 `SystemMessage`，确保系统规则不可篡改；模型可调用工具需做白名单管控与权限校验，防范诱导式越权操作。

- **上下文与成本管控**：借助 `ChatMemory` 实现消息窗口化管理，严控上下文总规模；长期用户信息优先采用「摘要压缩 + RAG 动态检索」方案；配置 `maxOutputTokens` 等参数，筑牢成本管控阈值。

- **工具调用规范**：工具执行结果统一返回 JSON 格式；二次请求需完整携带 `AiMessage`（工具调用请求）与 `ToolExecutionResultMessage`（工具执行结果）；工具必须实现幂等性，规避重复执行风险。

## 参考文档

- [LangChain4j 官方文档 - Chat and Language Models](https://docs.langchain4j.dev/tutorials/chat-and-language-models)
